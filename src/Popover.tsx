...
class Popover extends React.Component<PopoverProps, PopoverState> {
  ...
  public render() {
    ...
    if ((this.props.isOpen || isTransitioningToClosed) && this.popoverDiv && popoverInfo) {
      const getContent = (args: PopoverInfo): JSX.Element => (typeof content === 'function' ? content(args) : content);

      popoverContent = (
        <PopoverPortal element={this.popoverDiv} container={this.props.contentDestination || window.document.body} children={getContent(popoverInfo)} />
      );
    }
    ...
    this.renderWithPosition({ position: this.positionOrder[positionIndex], targetRect: this.target.current.getBoundingClientRect() }, (violation, rect) => {
      ...
      if (violation && !disableReposition && !(typeof contentLocation === 'object')) {
        ...
        if (contentLocation) {
          ...
          ({ top, left } =
            typeof contentLocation === 'function' ? contentLocation({ targetRect, popoverRect, position, align, nudgedLeft, nudgedTop }) : contentLocation);
